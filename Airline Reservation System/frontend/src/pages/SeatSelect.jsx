...
export default function SeatSelect() {
    ...
    const fetchALLModels = async () => {
      try {
        const res = await axios.get(
          "http://localhost:8000/seat_select/" + flight_id + "/" + customer_id
        );
        setseats(res.data);
        console.log(res);
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:8000/seat_select/" +
          CurrentSeat.seat_id +
          "/" +
          flight_id +
          "/" +
  ...
  return (
    <>
      ...
      <Background />
      <br></br>
      <div className="seatsel">
        <div className="glassBox5">
          <h1>Seat Selection</h1>
          <Container className="allBoxes3" sx={{ marginTop: 4 }}>
            <FormControl>
              <RadioGroup
                row
